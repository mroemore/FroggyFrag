...
bool updateScreenDimensions(Rectangle *sourceRect, Rectangle *destRect,
                            int *screenWidth, int *screenHeight,
                            float *virtualRatio);

// ShaderManager functions
void addShaderPath(ShaderManager *sm, char *newPath);
void initShaderManager(ShaderManager *sm, TextBox *notificationTB,
                       char *folder);
void cleanupShaderManager(ShaderManager *sm);
...
int main(void) {
  ...
  if (fontSystem.texture.id == 0) {
    fontSystem = GetFontDefault();
  ...
  ShaderManager sm;
  TextBox *notificationTB = createSettingsNotificationBox(
      10, -50, 400, 50, "default", (Color){36, 36, 35, 200},
      (Color){245, 203, 92, 200});
  addDrawableToRoot((Drawable *)notificationTB);
  initShaderManager(&sm, notificationTB, getConfigValueString("shaderFolder"));

  int termWidth = screenWidth - 100;
  int termHeight = screenHeight - 100;
  g_terminal = createTerminalOverlay(NULL, 50, 50, termWidth, termHeight,
      getConfigValueString("systemFontPath"), 14);
  if(g_terminal) {
    addDrawableToRoot((Drawable *)g_terminal);
  }

  Button *terminalBtn = createButton(NULL, screenWidth - 120, 40, 100, 30,
      "Terminal", (Color){60, 60, 70, 200}, (Color){200, 200, 200, 255},
      fontSystem, 16, onTerminalToggle, NULL);
  if(terminalBtn) {
    addDrawableToRoot((Drawable *)terminalBtn);
  }
  ...
  // (Color){ 0, 0, 0, 170 }, (Color){ 200, 200, 200, 200 });
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    if (updateScreenDimensions(&sourceRec, &destRec, &screenWidth,
                               &screenHeight, &virtualRatio)) {
    ...
    if (IsKeyReleased(KEY_A)) {
      ...
      autoReload ? newSettingsInfo(sm.notification, "auto reload enabled.")
                 : newSettingsInfo(sm.notification, "auto reload disabled.");
    }
    ...
    if(g_terminal && isTerminalOverlayVisible(g_terminal)) {
    ...
    SetShaderValue(sm.current, GetShaderLocation(sm.current, "iTime"), &time,
                   SHADER_UNIFORM_FLOAT);
    SetShaderValue(sm.current, GetShaderLocation(sm.current, "mousePosition"),
                   &mousePosition, SHADER_UNIFORM_VEC2);

    SetShaderValue(sm.current,
                   GetShaderLocation(sm.current, "windowResolution"),
                   &windowResolution, SHADER_UNIFORM_VEC2);
  ...
  cleanupShaderManager(&sm);
  if(g_terminal) {
    destroyTerminalOverlay(g_terminal);
  }
  if (bgTex.id != 0) UnloadTexture(bgTex);
  if (target.id != 0) UnloadRenderTexture(target);
  if (fontSystem.texture.id != 0) UnloadFont(fontSystem);
  CloseWindow();
}
...
void screenshot(ShaderManager *sm) {
  ...
  if (originalWorkingDirectory == NULL) {
    printf("Error: Failed to duplicate working directory path\n");
    return;
  }

  bool chDirSuccess =
      ChangeDirectory(getConfigValueString("screenshotsFolder"));
  if (chDirSuccess) {
    int count = 0;
    const char **currentShaderName =
        TextSplit(sm->shaderPaths[sm->currentShaderIndex], '/', &count);

    int screenshotIndex = 0;
    char screenshotFileName[255];
    snprintf(screenshotFileName, 255, "%s_%i.png", currentShaderName[count - 1],
             screenshotIndex);

    while (FileExists(screenshotFileName)) {
      screenshotIndex++;
      snprintf(screenshotFileName, 255, "%s_%i.png",
               currentShaderName[count - 1], screenshotIndex);
    }
    TakeScreenshot(screenshotFileName);
    char settingsMessage[255];
    snprintf(settingsMessage, 255, "Screenshot \"%s\" saved!",
             screenshotFileName);
...
void initShaderManager(ShaderManager *sm, TextBox *notificationTB,
  ...
  sm->errors = createMessageBuffer(NULL, 10, 10, 800, 800, BLACK, WHITE,
                                   getConfigValueString("systemFont"), 14);
...
void swapOrReloadShader(ShaderManager *sm, int index) {
  ...
  if (loadRequired) {
    // open and overwrite file for dumping potential GLSL errors, redirecting
    // stdout to it.
    ...
    if (IsShaderValid(newShader)) {
      ...
      char settingsMessage[255];
      snprintf(settingsMessage, 255, "Shader \"%s\" loaded.",
               sm->shaderPaths[sm->currentShaderIndex]);
      newSettingsInfo(sm->notification, settingsMessage);
