...
return vec3(clamp(f(+4.0767416621f * l - 3.3077115913f * m + 0.2309699292f * s),0.,1.),
			clamp(f(-1.2684380046f * l + 2.6097574011f * m - 0.3413193965f * s),0.,1.),
			clamp(f(-0.0041960863f * l - 0.7034186147f * m + 1.7076147010f * s),0.,1.));
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    float sscol = smoothstep(0.0,1.0,bright);
    ...
    float lo2 = osin(uv.x*shmx + TWO_PI*((t+(0.5*uv.x))/5.));
    float bright2 = clamp((pnabz * log(.00785+lo2))*max(brs,br),0.,1.);
    float sscol2 = smoothstep(0.0,1.0,bright2);
    
    vec3 fc2 = vec3(
        mix(colours[2].x, colours[3].x, sscol2),
        mix(colours[2].y, colours[3].y, sscol2),
        mix(colours[2].z, colours[3].z, sscol2));
...
#endif
    float dbg_dtr = dither(uv, 8.);
    if ((bright > 0.0 && bright < 0.2) && dr == 0.) {
        c = mix(vec3(1,0,1),vec3(0,.85,.80),dbg_dtr);
    }
    if ((bright2 > 0.0 && bright2 < 0.2) && dr == 1.) {
        c = mix(vec3(0.4,.85,0.35),vec3(1,1,0),dbg_dtr);
    }
#ifdef ANDROID_SHADER_EDITOR
