...
float cellular(vec2 p) {
    ...
    for (int j=-1; j<=1; j++ ) {
        for (int i=-1; i<=1; i++ ) {
            ...
            //point *= c_dist;
            point = 0.5 + 0.5*sin(6.2831*point + iTime);
...
void main() {
    ...
    uv *= 7. + (2. * c_dist);//((0.5+0.5*sin(c_dist))- (0.5+0.5*cos(c_dist))); 
	//uv = abs(uv);
  
    //gl_FragColor = vec4(vec2(v),v/2.,1.0);
    float cell = cellular(uv);
...
//     float avgc = (ac1.r + ac1.g + ac1.b)/3.;
//     avgc -= 0.5;
//     ac2+= avgc * .95;
//     avgc = (ac2.r + ac2.g + ac2.b)/3.;
//     avgc -= 0.5;
//     ac2+= avgc * .95;
	c*= ac2 + ac1;
    if(test_flag){
