...
- **Issues:**
  - ~~[HIGH] Old text lines never freed - memory leak~~ **FIXED**
...
- **Issues:**
  - ~~[CRITICAL] Memory leak: CVT_STRING case malloc's but no cleanup of previous value~~ **FIXED**
...
- **Called by:** `initGlobalConf()`
- **Issues:**
  - ~~[MEDIUM] `screenshotsFolder`, `imagesFolder` not initialized but freed in `freeConfig()`~~ **FIXED**
...
16. **conf.c:initDefaultConf()** - Changed string literals to strdup'd copies so they can be safely freed; initialized screenshotsFolder and imagesFolder to NULL
17. **conf.c:setConfigValue()** - Added free for old string value before assigning new malloc'd value to prevent memory leaks
18. **main.c:initShaderManager()** - Added UnloadDirectoryFiles(pl) to free directory listing memory
19. **main.c:main()** - Fixed getConfigValueString memory leaks for systemFontPath and backgroundImagePath
20. **gui.c:newSettingsInfo()** - Added cleanup of old text lines before replacing with new text to prevent memory leak

### Memory Leak Fix Summary

Valgrind results after all fixes:
- **Before:** 33,529,856 bytes definitely lost, 33,464,320 bytes indirectly lost
- **After:** 0 bytes definitely lost, 0 bytes indirectly lost
- All application-level memory leaks have been fixed
