...
- **Issues:**
  - [CRITICAL] Line 56: Bug - `screenHeight` initialized with `getConfigValueInt("screenW")` instead of `"screenH"`
  - [HIGH] No cleanup of resources on exit (shaders, textures, fonts, render textures)
  - [HIGH] `originalWorkingDirectory` in `screenshot()` leaked (strdup without free)
...
- **Issues:**
  - [CRITICAL] Memory leak: `strdup(GetWorkingDirectory())` result never freed
...
- **Issues:**
  - [HIGH] No NULL check on `strdup` return
...
- **Issues:**
  ...
  - [MEDIUM] Hardcoded `.glsl` extension in `rescanDirectory()` but uses config here
...
- **Issues:**
  - [CRITICAL] `freopen("/dev/tty", "w", stdout)` - non-portable (Windows incompatible)
  - [HIGH] `errFile` loaded with `LoadFileText` but never freed with `UnloadFileText`
...
- **Issues:**
  - [HIGH] Hardcoded `.glsl` extension instead of using config value
...
- **Issues:**
  - [CRITICAL] Memory leak: `strdup(msg)` assigned to `mb->messages[mb->index]` but old message at that index never freed
...
- **Issues:**
  ...
  - [CRITICAL] `AnimationChain` structs (`opacityFade`, `positionSlide`, etc.) are stack-allocated but referenced by pointer in group - undefined behavior after function returns
...
- **Issues:**
  - [CRITICAL] Same stack allocation bug as `createSettingsNotificationBox` - AnimationChain structs are local
...
- **Issues:**
  - [HIGH] `segments[0].animate` called without checking if segment exists
...
- **Issues:**
  ...
  - [HIGH] `strncpy` doesn't null-terminate if source >= MAX_PATH_LENGTH
...
- **Issues:**
  ...
  - [MEDIUM] Doesn't free `shaderFileExtension`
...
- **Called by:** Various
- **Issues:**
  - [MEDIUM] Return -1/-1.0f/NULL on error - indistinguishable from valid values
...
- **Issues:**
  ...
  - [HIGH] `strncpy` may not null-terminate
...
1. **Fix screen height bug** - `main.c:56`
   ```c
   // WRONG:
...
2. **Fix stack-use-after-return** - AnimationChain in gui.c
...
2. Add NULL checks after all allocations
3. Fix buffer overflow risks with bounded operations
4. Make `freopen` portable or add platform abstraction
